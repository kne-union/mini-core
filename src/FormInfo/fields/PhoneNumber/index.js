...
import {Divider, Icon} from "@kne/antd-taro";
...
const PhoneNumberFieldInner = withFetch(({data, value = {}, onChange, req, ...props}) => {
    ...
    return <View className={classnames(style['flex'], {
        ...
        <View className={style['code']} onClick={() => setVisible(true)}>
          {getCountryCode({code: value.code, data})}<Icon type="arrow-bold-down" className="iconfont"/>
        </View>
