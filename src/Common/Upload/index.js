...
const Upload = ({
    ...
    const {apis: apisBase, getUploadHeader} = usePreset();
    ...
    const onUpload = () => {
        ...
        if (!(maxLength === 1 || multiple !== true) && uploadingList.length > allowCount) {
            onError({
        ...
        Promise.resolve(getUploadHeader?.()).then((header) => {
            const doUpload = (res) => {
                const tempFilePaths = res.tempFiles[0];
                const {size, name} = tempFilePaths;
                if (size > fileSize * 1024 * 1024) {
                    onError({
                        message: `文件${name}不能超过${fileSize}MB!`, file: tempFilePaths
                    });
                    return;
                }
                const uuid = uniqueId();
                if (maxLength === 1 || multiple === false) {
                    setUploadingList([{
                        uuid, type: "uploading", fileName: name, originalName: name
                    }]);
                } else {
            ...
            if (isWXWork()) {
                Taro.chooseImage({
                    count: 1, success: doUpload
                })
                return;
            }
            Taro.chooseMessageFile({
                count: 1, type: chooseFileType || "all", success: doUpload
            })
        })
    }

    const tipsText = renderTips(`支持扩展名${accept
        .map((str) => str.replace(/^\./, ""))
...
Upload.defaultProps = {
    ...
    accept: [".pdf", ".jpg", ".png", ".jpeg", ".doc", ".docx", ".xls", ".xlsx", ".html", ".msg", ".zip",],
