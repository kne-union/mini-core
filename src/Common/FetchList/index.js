...
const FetchList = withFetch(({data, reload, loadMore, showTotalCount, requestParams, emptyDescription, withFilter, isComplete, needMergeList, children, hasBorder=true}) => {
  ...
  return (
    <View className={hasBorder?style["border-box"]:""}>
      {showTotalCount && data?.totalCount && isComplete ? <View className={withFilter ? style['result-info'] : style['bottom-info-view']}>搜索到{data?.totalCount}条结果</View> : null}
      ...
      {!hasMore && data?.totalCount && !loading && isComplete ? <View className={style['bottom-info-view']}>到底了</View> : null}
